...
func (h *handler) Update(ctx context.Context, req *jwt.Secret) (*jwt.Secret, error) {
	logger := log.With().Str("method", "update").Logger()

	if req == nil {
		return &jwt.Secret{}, status.New(codes.Internal, gerrors.ErrNullRequest{}.Error()).Err()
	}

	result, err := h.jwt.UpdateSecret(ctx, *req)
	if err != nil {
