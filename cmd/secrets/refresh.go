...
func (h *handler) Refresh(ctx context.Context, req *types.Empty) (*jwt.SecretList, error) {
	logger := log.With().Str("method", "refresh").Logger()

	if req == nil {
		return &jwt.SecretList{}, status.New(codes.Internal, gerrors.ErrNullRequest{}.Error()).Err()
	}

	result, err := h.jwt.RefreshSecret(ctx)
	if err != nil {
